...
const GoogleMapsComponent: React.FC = () => {
    ...
    return (
        <div className={`${lato.className}`}>
            <h1 className={`text-3xl uppercase mt-5 mb-5 font-semibold`}>Puntos Verdes</h1>
            {isLoaded ? (
                <GoogleMap mapContainerStyle={containerStyle} center={userLocation || { lat: -29.8846148, lng: -61.9427404 }} zoom={userLocation ? 20 : 14}>
                    {wasteDumpLocations.map((dump, index) => (
                        <Marker key={index} position={{ lat: -29.8846148, lng: -61.9427404 }} onClick={() => handleMarkerClick(dump)}>
                            {selectedMarker === dump && (
