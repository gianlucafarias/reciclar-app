        ...
        <>
            <section className="flex min-h-[88vh] justify-center items-center">
                <div className=" border-black/10 border-2  shadow-lg shadow-black/10   w-full  m-4 md:m-auto p-4 rounded-lg">
                    <h1 className="font-semibold text-2xl text-center mb-5">Bienvenido</h1>
                    <form autoComplete="false" className="flex flex-col gap-2" onSubmit={handleLogin}>
                        <label htmlFor="Email">Correo Electronico</label>
                        <input type="email" placeholder="nombre@correo.com" onChange={(e) => setEmail(e.target.value)} className=" bg-transparent fill-none border-2  border-black/20 duration-200 focus:border-green-700 text-black p-2 focus:outline-none  rounded-lg  " autoComplete="off" />
                        <label htmlFor="Password">Contraseña</label>
                        <div className="flex justify-between fill-none items-center border-2 rounded-lg border-black/20 p-2 ">
                            <input type={`${showPassword ? "text" : "password"}`} placeholder="Contraseña" className=" w-[90%] bg-transparent focus:outline-none" onChange={(e) => setPassword(e.target.value)} />
                            {showPassword ? <EyeOff onClick={() => setShowPassword(!showPassword)} /> : <Eye onClick={() => setShowPassword(!showPassword)} />}
                        </div>
                        {password.length < 0 && <span className=" text-red-500">La contraseña debe tener al menos 8 caracteres.</span>}
