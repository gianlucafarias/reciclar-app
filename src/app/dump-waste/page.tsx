...
const Page: React.FC = () => {
    ...
    const scanImage = async () => {
        try {
            ...
            const prompt = "¿Esto te parece un cubo de basura? Responde con si o no.";
    ...
    const getDustBinType = async () => {
        try {
            const genAI = new GoogleGenerativeAI(process.env.NEXT_PUBLIC_GEMINI_API_KEY!);
            const model = genAI.getGenerativeModel({ model: "gemini-pro-vision", safetySettings: safetySettings });
            const prompt = "¿Esto parece un cesto de basura? Responde con true o false.  ";
            const formatMatch = photoData.match(/^data:(image\/(\w+));base64,/);
            if (!formatMatch) {
                console.error("Formato de imagen no soportado");
                Toast.ErrorShowToast("Formato de imagen no soportado");
                return;
            }
    ...
    return (
        <div className="flex relative flex-col  mt-4 w-full mb-40">
            <h1 className=" flex items-start justify-start mb-7 text-3xl font-bold text-start">Cesto de residuos</h1>
            <div>
                    ...
                    {aiData && (
                        <div className="  bg-black/5 shadow-lg w-full mt-12 mb-28 rounded-2xl border-2 border-black/10">
                            <div className="flex flex-col gap-4  p-4">
                                <h1 className="text-2xl  font-bold">¿Es un basurero? {aiData}</h1>
                                <h1 className=" text-2xl font-bold">¿Es un basurero de residuos secos? {isDryWaste}</h1>
                                <h1 className=" text-xl opacity-60 font-bold uppercase">esReciclable = {recycle}</h1>
                                <h1>nombreresido : {wasteName}</h1>
                                <h1>tipo {wasteType}</h1>
                                <h1>Material : {material}</h1>
                                <h1>Total reciclado: {totalwaste}</h1>
                                <h1>residuo seco: {dryWaste}</h1>
                                {aiData.trim().toLowerCase() === "si" && isDryWaste.trim().toLowerCase() === dryWaste?.trim().toLowerCase() ? (
                                    ...
                                    <button className=" flex justify-center items-center gap-3 bg-red-600 text-white p-5 rounded-lg">
                                        <Coins />
                                        Basurero no detectado {dryWaste?.trim().toLowerCase() ? "Dry Waste" : "Wet Waste"}{" "}
                                    </button>
                                )}
                                ...
                                <div className={`absolute bg-white shadow-md shadow-black/10 border-2  border-black/10 z-50 w-[90%] m-auto right-0 left-0 h-96 rounded-lg top-16 ${claimRewards ? "scale-100" : "scale-0"} duration-200`}>
                                    ...
                                    {dumpLoading ? (
                                        ...
                                        <div className="flex justify-center items-center  flex-col">
                                            ...
                                            <h1 className=" text-2xl font-bold capitalize">Felicitaciones!</h1>
